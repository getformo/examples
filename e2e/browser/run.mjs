...
//
// Usage: node run.mjs <sdkPackageDir>   (expects anvil on :8545)
...
if (!sdkDir) { console.error("usage: node run.mjs <sdkPackageDir>"); process.exit(2); }
const sdkPath = join(sdkDir, "dist/index.umd.min.js");
...
const chrome = [process.env.CHROME_PATH, "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", "google-chrome-stable", "google-chrome", "chromium-browser", "chromium", "chrome"].filter(Boolean)
...
// `providers` is public API on every version; the registry internals are not.
results.push(["discovered", await evaluate("window.__ready.then(f => f.providers.map(d => d.info.rdns))")]);
