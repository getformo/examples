...
//
// Usage: node behaviours.mjs <sdkPackageDir> [filter]
...
const [, , sdkDir, filter] = process.argv;
if (!sdkDir) { console.error("usage: node behaviours.mjs <sdkPackageDir> [filter]"); process.exit(2); }

// wagmi scenarios run against the page-router example (wagmi v2 + viem),
// everything else against with-react (plain EIP-1193).
const EXAMPLE = { wagmi: "with-next-page-router" };
const exampleFor = (mode) => join(root, EXAMPLE[mode] ?? "with-react");
