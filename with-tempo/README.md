# Tempo + Formo SDK Example

This is an example application demonstrating the integration of [Tempo Accounts SDK](https://docs.tempo.xyz/accounts) with [Formo Analytics SDK](https://formo.so) using [Wagmi](https://wagmi.sh).

## Features

- **Tempo Wallet Integration** - Connect using Tempo's account system with passkey authentication
- **Formo Analytics** - Automatic event tracking for wallet interactions
- **Token Balance** - Real-time pathUSD token balance display via Tempo SDK
- **Token Transfers** - Send pathUSD transfers using Tempo's token actions
- **Full Event Coverage**:
  - Connect/Disconnect events
  - Signature events (personal_sign)
  - Transaction events
  - Chain switch events
  - Custom events
  - Page view events
...
└── wagmi.ts             # Wagmi configuration with Tempo wallet connector
```
...
- **Signature events** - When a user signs a message
...
## Tempo SDK Integration

The Tempo Accounts SDK is configured in `src/wagmi.ts`:
...
- **Token Balance** - `Hooks.token.useGetBalance` from `wagmi/tempo` for real-time balance queries
- **Token Transfers** - `Actions.token.transfer.call` from `viem/tempo` for pathUSD transfers
- **Passkey Auth** - Tempo wallet uses WebAuthn for secure authentication
...
- **Tempo** - Tempo mainnet
- **Tempo Moderato** - Tempo testnet

## Resources

- [Tempo Documentation](https://docs.tempo.xyz/accounts)
- [Tempo GitHub](https://github.com/tempoxyz/accounts)
