...
import { useFormo, TransactionStatus } from "@formo/analytics";
...
export function TransferFunds() {
  ...
  async function handleOnTransfer() {
    ...
    try {
      ...
      // Use the raw string the user typed — avoids float-to-string rounding.
      const txn = await wallet.send(recipient, "usdxm", amountInput);
      setExplorerLink(txn.explorerLink);

      // Formo: the transaction was broadcast on-chain (carries the tx hash).
      formo?.transaction({
      ...
      // Formo: a custom event capturing the transfer details.
      formo?.track("crossmint_transfer", {
        token: "usdxm",
      ...
      if (err instanceof Error && err.name === "AuthRejectedError") {
        // The user explicitly cancelled the wallet prompt — emit REJECTED.
        formo?.transaction({
